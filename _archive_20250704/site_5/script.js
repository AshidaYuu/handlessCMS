...
async function loadNewsFromSanity() {
    ...
    // キャッシュが有効な場合は即座に表示
    if (newsCache && newsCacheTime && (Date.now() - newsCacheTime < CACHE_DURATION)) {
...
function displayNews(posts) {
    ...
    // ローディングを非表示（元から非表示にしているので不要だが念のため）
    if (newsLoadingEl) {
        newsLoadingEl.style.display = 'none';
    }
    
    // ニュース一覧を生成
    const newsHTML = posts.map((post, index) => {
        const date = new Date(post.publishedAt);
        const formattedDate = `${date.getFullYear()}.${(date.getMonth() + 1).toString().padStart(2, '0')}.${date.getDate().toString().padStart(2, '0')}`;
        const slug = post.slug ? post.slug.current : '';
        
        return `
            <article class="news-item news-fade-in" style="animation-delay: ${index * 0.1}s">
...
document.addEventListener('DOMContentLoaded', function() {
    // ニュースセクションがページ内にある場合のみ読み込み
    const newsSection = document.getElementById('news-section');
    if (newsSection) {
        // Sanityからニュースを即座に読み込み
